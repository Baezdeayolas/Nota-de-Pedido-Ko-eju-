...
function generatePdfBytes(orderState) {
  ...
  function drawTableHeader() {
    currentLines.push({ x: marginLeft, y, size: 11, bold: true, font: 'mono', text: padCol('CÓDIGO', 14) });
    currentLines.push({ x: marginLeft + 92, y, size: 11, bold: true, font: 'sans', text: 'DESCRIPCIÓN' });
    currentLines.push({ x: 300, y, size: 11, bold: true, font: 'mono', text: padCol('CANT.', 5, true) });
    currentLines.push({ x: 337, y, size: 11, bold: true, font: 'mono', text: padCol('LISTA', 5, true) });
    currentLines.push({ x: 374, y, size: 11, bold: true, font: 'mono', text: padCol('PRECIO', 12, true) });
    currentLines.push({ x: 458, y, size: 11, bold: true, font: 'mono', text: padCol('SUBTOTAL', 13, true) });
    y -= 12;
  ...
  currentLines.push({ x: rightColX, y, size: 12, bold: false, font: 'sans', text: truncate(`Condición de venta: ${orderState.condicion || '-'}`, 33) });
  y -= 17;
  currentLines.push({ x: marginLeft, y, size: 12, bold: false, font: 'sans', text: truncate(`Cliente: ${orderState.cliente || '-'}`, 38) });
  currentLines.push({ x: rightColX, y, size: 12, bold: false, font: 'sans', text: `Lista de precio: M${orderState.tier}` });
  y -= 17;
  currentLines.push({ x: marginLeft, y, size: 12, bold: false, font: 'sans', text: `Código cliente: ${orderState.clienteCodigo || '-'}` });
  currentLines.push({ x: rightColX, y, size: 12, bold: false, font: 'sans', text: `Fecha: ${new Date().toLocaleDateString('es-PY')}` });
  y -= 26;
  drawTableHeader();
  ...
  orderState.lines.forEach((line) => {
    ...
    ensureSpace();
    currentLines.push({ x: marginLeft, y, size: 10.5, bold: false, font: 'mono', text: padCol(item.c, 14) });
    currentLines.push({ x: marginLeft + 92, y, size: 10.5, bold: false, font: 'sans', text: truncate(item.d, 26) });
    currentLines.push({ x: 300, y, size: 10.5, bold: false, font: 'mono', text: padCol(String(line.qty), 5, true) });
    currentLines.push({ x: 337, y, size: 10.5, bold: false, font: 'mono', text: padCol('M' + line.tier, 5, true) });
    currentLines.push({ x: 374, y, size: 10.5, bold: false, font: 'mono', text: padCol(fmtMoney(price), 12, true) });
    currentLines.push({ x: 458, y, size: 10.5, bold: false, font: 'mono', text: padCol(fmtMoney(subtotal), 13, true) });
    y -= rowHeight;
  });
  ...
  y -= 22;
  currentLines.push({ x: 374, y, size: 14, bold: true, font: 'sans', text: 'TOTAL' });
  currentLines.push({ x: 458, y, size: 14, bold: true, font: 'mono', text: padCol('Gs. ' + fmtMoney(grandTotal()), 16, true) });
