...
document.getElementById('btn-clear').addEventListener('click', () => {
  ...
  state = { tier: 1, vendedor: keepVendedor, cliente: '', clienteCodigo: '', clienteCiudad: '', condicion: '', lines: [], editingId: null };
  saveState();
...
function generatePdfBytes(orderState) {
  ...
  const colDto = 507; // borde derecho de Dto.
  ...
  function ensureSpace() {
    if (y - rowHeight < marginBottom) {
      ...
      pageNum += 1;
      y = marginTop;
      addRight(marginRight, y, 8, false, `Página ${pageNum}`);
      y -= 18;
      drawTableHeader();
    }
  }
  ...
  addText(marginLeft, y, 8, false, "Ko'eju Rory S.A.");
  const ahora = new Date();
  const fechaHora = `${ahora.toLocaleDateString('es-PY')} ${ahora.toLocaleTimeString('es-PY', { hour: '2-digit', minute: '2-digit' })}`;
  addRight(marginRight, y, 8, false, fechaHora);
  y -= 10;
  addText(marginLeft, y, 6, false, `Emitido por ${orderState.vendedor || '-'}`);
  addRight(marginRight, y, 8, false, `${pageNum}`);
  y -= 22;
  ...
  orderState.lines.forEach((line) => {
    ...
    addRight(colDto, y, 7, false, '0,00%');
  ...
  y = boxBottom - 14;
  addText(marginLeft, y, 7, false, `${orderState.vendedor || '-'} ${pageNum}`);
  y -= 20;
  addText(marginLeft, y, 8, false, 'Pedido sujeto a variación de Precios y Stock.');
